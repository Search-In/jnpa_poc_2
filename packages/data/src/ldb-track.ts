  ...
  if (Array.isArray(obj)) {
    ...
    return obj.slice().sort((a, b) => {
      const score = (o: LdbSearchObject) =>
        ...
        (o.trackLog?.length ?? 0) +
        (hasVessel(o.vesselStatusExportDpt) ||
        hasVessel(o.vesselStatusExportArv)
          ? 5
          : 0) +
        (hasVessel(o.vesselStatusImportArv) ||
        hasVessel(o.vesselStatusImportDpt)
          ? 5
          : 0);
...
function hasVessel(
  v: LdbVesselStatus | Record<string, never> | undefined,
): v is LdbVesselStatus {
  return (
    !!v &&
    typeof v === "object" &&
    typeof (v as LdbVesselStatus).eventname === "string"
  );
}

function mapVesselStatus(
  raw: LdbVesselStatus | Record<string, never> | undefined,
): NldsVoyageEvent | undefined {
...
function exportVoyageFrom(obj: LdbSearchObject): NldsVoyageEvent[] {
  return [
    mapVesselStatus(obj.vesselStatusExportDpt),
    mapVesselStatus(obj.vesselStatusExportArv),
  ].filter((e): e is NldsVoyageEvent => !!e);
}

/** Import: arrival first (NLDS primary), then departure if present. */
function importVoyageFrom(obj: LdbSearchObject): NldsVoyageEvent[] {
  return [
    mapVesselStatus(obj.vesselStatusImportArv),
    mapVesselStatus(obj.vesselStatusImportDpt),
  ].filter((e): e is NldsVoyageEvent => !!e);
}
...
export function normalizeLdbSearch(
  raw: LdbSearchResponse,
  containerNo: string,
): NldsContainerTrack {
  ...
  return {
    ...
    importVoyage,
    found:
      stops.length > 0 || exportVoyage.length > 0 || importVoyage.length > 0,
  };
}
 ...
 * Errors thrown for the UI are operator-facing (no proxy/HTTP jargon).
 */
export async function fetchLdbContainerTrack(
  ...
  const unavailable = () => {
    throw new Error('Container track data not available on NLDS.');
  };
  ...
  try {
    ...
    unavailable();
    return emptyTrack(norm); // unreachable — satisfies TS control flow
  }
  if (!res.ok) {
    unavailable();
  }
  const text = await res.text();
  // Missing /ldb proxy (SPA HTML fallback) or any non-JSON body → same UX message.
