...
describe('ScenarioEngine', () => {
  it('runs CGO-1/2/3 + LANE-ASSIGN with before/after deltas + actions', () => {
    const e = engine();
  ...
  it('CGO-2 emits a real cross-twin DeferredArrivalWindow onto the shared topic', () => {
    const bus = new InMemoryEventBus();
    const received: CloudEvent[] = [];
    bus.subscribe(CROSS_TWIN_TOPIC, (ev) => received.push(ev));

    const r = engine(bus).run('CGO-2', { gateId: 'GTI-G2' });
  ...
  it('is deterministic: same scenario+seed → identical cross-twin correlationId', () => {
    const a = engine().run('CGO-2');
    const b = engine().run('CGO-2');
